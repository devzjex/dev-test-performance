...
export default function ReviewApp({ initialDataReviews }) {
  ...
  const [countData, setCountData] = useState(initialDataReviews.dataSummaryReview.data);
  ...
  const [prosList, setProsList] = useState(
    initialDataReviews.dataSummaryReview.data.pros_app
      ? initialDataReviews.dataSummaryReview.data.pros_app.split('\n').map((item) => item.replace(/^- /, '').trim())
      : [],
  );
  const [consList, setConsList] = useState(
    initialDataReviews.dataSummaryReview.data.cons_app
  ...
  useEffect(() => {
    const handleResize = () => {
      const isSmall = window.innerWidth <= 1250;
      setIsContentVisible(!isSmall);
      setIsSize(isSmall);
    };
  ...
  const handleChangeSort = useCallback(
    async (value) => {
      ...
      const newIsDeleted = sortValue === 1 ? true : null;
      const newIsArchived = sortValue === 2 ? true : null;

      setIsDeleted(newIsDeleted);
      setIsArchived(newIsArchived);

      const typeReview = sortValue === 1 ? 'deleted' : sortValue === 2 ? 'archived' : '';
      try {
        const [summaryRes, listRes] = await Promise.all([
          DetailAppApiService.getReviewAppInfoSummary(idDetail, typeReview),
          getReviewListDetailApp(
            idDetail,
            newIsDeleted,
            PAGE_DEFAULT_REVIEW,
